    ...
    for i in range(len(data)-1):
        ...
        if VI:
            if not (data['V-I'][i] > VI[0] and data['V-I'][i] < VI[1]):
                data = data.drop(data.index[[i]])
        if alt_az:
            if not Angle(body.alt,unit=u.hour) > Angle(alt_az[0],unit=u.hour) and \
            not Angle(body.alt,unit=u.hour) < Angle(alt_az[1],unit=u.hour) and \
            not Angle(body.az,unit=u.degree) > Angle(alt_az[2],unit=u.degree) and \
            not Angle(body.az,unit=u.degree) < Angle(alt_az[3],unit=u.degree):
                data = data.drop(data.index[[i]])
    return data
